...
to run the experiment, first copy the ```.conf``` files from inside the 'conf' of this repo directory to ```/usr/local/etc/``` then run the python script that is inside the 'advanced' directory of this repo.
